...
export function CSprite(src, _from, _x, _y, _interactive) {
    let mc;
    let anchorX = _x || 0;
    let anchorY = _y || 0;
 ...
 * 创建Graphics矩形
 */
export function CdrawRect(__width, __height, __options) {
    let options = {
 ...
 * 创建Graphics圆形
 */
export function CdrawCircle(__radius, __options) {
    const options = {
        lineWeight: __options.lineWeight || 0, //边线的粗细度，默认为0
        lineColor: __options.lineColor || '0xffffff', //边线的颜色
        lineAlpha: __options.lineAlpha || 0, //边线透明度
 ...
 * 创建普通按钮
 */
export function CBtnText(__txt, __width, __height, __options) {
    const options = {
        lineWeight: __options.lineWeight || 0, //边线的粗细度，默认为0
        lineColor: __options.lineColor || '0xffffff', //边线的颜色
        lineAlpha: __options.lineAlpha || 0, //边线透明度
    ...
    let txt = new PIXI.Text(__txt, fontStyle);
    mc.addChild(txt);
 ...
 * 创建普通圆形按钮
 */
export function CBtnCircleText(__txt, __radius, __options) {
    const options = {
        lineWeight: __options.lineWeight || 0, //边线的粗细度，默认为0
        lineColor: __options.lineColor || '0xffffff', //边线的颜色
        lineAlpha: __options.lineAlpha || 0, //边线透明度
 ...
 * loop：是否循环播放
 */
export function setVideo(name, ext, sNum, tol, speed, onComplete, loop) {
    let VTextures = [],
        _mc;
    loop == undefined ? loop = false : loop;
    speed == undefined ? speed = .5 : speed;
    for (let i = sNum; i <= tol; i++) {
        let texture = PIXI.Texture.fromFrame(name + i + ext);
 ...
 * loop：是否循环
 */
export function VideoStop(vmc, videotolN, nowN, loop) {
    if (loop) {
...
export function onPerspective(_mc) {
    let scale = fl / (fl + _mc.zpos);
...
//设置Mc容器
export function setSpriteMc(parentmc, mcx, mcy) {
    let mc = new PIXI.Sprite();
...
//画虚线框
export function dashLineRect(__lineLength, __lineDistance, __totalWidth, __totalHeight, option) {
    let graphics;
